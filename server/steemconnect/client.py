...
class Client:
    ...
    def get_login_url(self, redirect_uri, scope,
        ...
        return urljoin(
            self.oauth_base_url,
            "authorize?" + urllib.urlencode(params))
    ...
    def get_access_token(self, code):
        ...
        r = requests.post(
            urljoin(self.sc2_api_base_url, "oauth2/token/"),
            data=post_data
        )
    ...
    def me(self):
        url = urljoin(self.sc2_api_base_url, "me/")
        r = requests.post(url, headers=self.headers)
        return r.json()

    @requires_access_token
    def broadcast(self, operations):
        url = urljoin(self.sc2_api_base_url, "broadcast/")
        data = {
